 ...
 * src/midi/magenta-sound-font/<soundfont name>, which both browser apps reach
 * through a symlink in their static folder.
 */
...
/*
One place for every rendered set: each app's static folder symlinks to it, so a
soundbank rendered once is played by all of them.
*/
const DEFAULT_BUILD_DIR = path.join(
  projectRoot, 'src/midi/magenta-sound-font'
)
...
async function main() {
  ...
  if (!soundfontArg) {
    throw new Error(
      'Usage: node tools/magenta/generate-magenta-sound-font.js ' +
